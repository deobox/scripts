...
read -e -p "-> Enter Client IP: " ipsrc; validateip "${ipsrc}";
...
iptables -A INPUT -p ${proto} -s ${ipsrc} --dport ${lport} -j ACCEPT;
iptables -A FORWARD -p ${proto} -s ${ipsrc} --dport ${rport} -d ${ipdst} -j ACCEPT;
...
sysctl -w net.ipv4.ip_forward=1;
iptables -C INPUT -p ${proto} -s ${ipsrc} --dport ${lport} -j ACCEPT &>/dev/null || iptables -A INPUT -p ${proto} -s ${ipsrc} --dport ${lport} -j ACCEPT;
iptables -C FORWARD -p ${proto} -s ${ipsrc} --dport ${rport} -d ${ipdst} -j ACCEPT &>/dev/null || iptables -A FORWARD -p ${proto} -s ${ipsrc} --dport ${rport} -d ${ipdst} -j ACCEPT;
